...
const containerMusicas = document.querySelector('#songs-container')
const paginacao = document.querySelector('#prev-and-next-container')

const apiURL = `https://api.lyrics.ovh`

const fetchMusicas = async term => {
...
form.addEventListener('submit', event => {
    ...
    const buscaTermo = inputBuscar.value.trim() //Pega o valor do input e joga dentro da variavel, o trim é para que ele desconsidere os espações caso o cliente coloque
    ...
    if(!buscaTermo){ //Essa função não deixa o usuário final deixar o campo input vazio, isso poderia ser resolvido de duas formas: Essa que vamos fazer e a outra seria dentro da tag input colocar um parâmetro chamado <input type="text" required>
        containerMusicas.innerHTML = `<li class="warning-message">Por favor, digite um termo válido</li>`
        return
    }

    fetchMusicas(buscaTermo)
})